...
MV_HOME=${MV_HOME:-"/usr/multiverse"}
...
##	strace -f -e trace=\!futex,gettimeofday,clock_gettime java \
	JIT_FLAGS="-XX:CompileThreshold=10000 -XX:+CITime -XX:+PrintCompilation"

	java \
		${JAVA_FLAGS} \
		-Dmultiverse.loggername=shell"${LOG_SUFFIX}" \
		multiverse.server.engine.ServerShell \
