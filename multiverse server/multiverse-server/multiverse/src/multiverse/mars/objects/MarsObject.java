...
public class MarsObject extends MVObject {
    ...
    public Set<MarsAbility.Entry> findAbilitiesByCategory(String category) {
        lock.lock();
        try {
            Set<MarsAbility.Entry> abilities = new HashSet<>();
    ...
    public void setAbilityMap(Map<String, MarsAbility.Entry> map) {
        lock.lock();
        try {
            abilityMap = new HashMap<>(map);
        }
        finally {
            lock.unlock();
        }
    }
    protected Map<String, MarsAbility.Entry> abilityMap = new HashMap<>();
    ...
    public Map<String, Cooldown.State> getCooldownStateMap() {
        lock.lock();
        try {
            return new HashMap<>(cooldownStateMap);
        }
        finally {
            lock.unlock();
        }
    ...
    public void setCooldownStateMap(Map<String, Cooldown.State> map) {
        lock.lock();
        try {
            cooldownStateMap = new HashMap<>(map);
        }
        finally {
            lock.unlock();
        }
    }
    protected Map<String, Cooldown.State> cooldownStateMap = new HashMap<>();
