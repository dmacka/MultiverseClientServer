...
public class MarsMob extends MarsObject {
    ...
    protected void init() {
        ...
        if (perceiver() == null) {
            ...
            MobilePerceiver<WMWorldNode> p = new MobilePerceiver<>(
    ...
    public Set<MarsItem> getEquippedItems() {
        lock.lock();
        try {
            return new HashSet<>(equipMap.values());
        } finally {
            lock.unlock();
        }
    }
    ...
    public void setEquipMap(Map<MarsEquipSlot, MarsItem> equipMap) {
        ...
        try {
            ...
            this.equipMap = new HashMap<>(equipMap);
        } finally {
            lock.unlock();
        }
    }
    ...
    public Map<MarsEquipSlot, MarsItem> getEquipMap() {
        lock.lock();
        try {
            return new HashMap<>(equipMap);
        } finally {
            lock.unlock();
        }
    }

    protected Map<MarsEquipSlot, MarsItem> equipMap = new HashMap<>();
    ...
    public LinkedList<MarsQuest> getQuestPrototypes() {
        lock.lock();
        try {
            return new LinkedList<>(questSet);
        } finally {
            lock.unlock();
        }
    }
    ...
    public Set<MarsQuest> getConcludableQuests() {
        lock.lock();
        try {
            return new HashSet<>(concludeSet);
        } finally {
            lock.unlock();
        }
    }
    ...
    public Set<QuestState> getQuestStates() {
        lock.lock();
        try {
            return new HashSet<>(questStateSet);
        } finally {
            lock.unlock();
        }
    }
...
//        return rv;
    ...
    Set<QuestState> questStateSet = new HashSet<>();
    ...
    public void setSkillMap(Map<MarsSkill, Integer> skillMap) {
        this.skillMap = new HashMap<>(skillMap);
    }
    ...
    // skill -> xp map
    private Map<MarsSkill, Integer> skillMap = new HashMap<>();
    ...
    public Set<MarsMob> getAttackers() {
        lock.lock();
        try {
            return new HashSet<>(dmgTable.keySet());
        } finally {
            lock.unlock();
        }
    }
    ...
    public Set<MarsSkill> getAttackerSkills(MarsMob attacker) {
        ...
        try {
            ...
            return new HashSet<>(attackerDmgMap.keySet());
        } finally {
            lock.unlock();
        }
    }
    ...
    Map<MarsMob, Map<MarsSkill, Integer>> dmgTable = new HashMap<>();
    ...
    protected void setActiveAbilities(Set<MarsAbility.State> abilities) {
        activeAbilities = new HashSet<>(abilities);
    }
    ...
    protected Set<MarsAbility.State> activeAbilities = new HashSet<>();
