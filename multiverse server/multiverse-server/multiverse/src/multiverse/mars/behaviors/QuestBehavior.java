...
public class QuestBehavior extends Behavior implements MessageCallback {
    ...
    public Collection<MarsQuest> getStartQuests() {
        lock.lock();
        try {
            return new LinkedList<>(startQuestsMap.values());
        }
        finally {
            lock.unlock();
        }
    ...
    public Collection<MarsQuest> getEndQuests() {
        lock.lock();
        try {
            return new LinkedList<>(endQuestsMap.values());
        }
        finally {
            lock.unlock();
        }
    ...
    private Map<String, MarsQuest> startQuestsMap = new HashMap<>();
    private Map<String, MarsQuest> endQuestsMap = new HashMap<>();
    
    // players who have asked for quest info, we keep track of what quest we gave them
    private Map<Long, MarsQuest> offeredQuestMap = new HashMap<>();
